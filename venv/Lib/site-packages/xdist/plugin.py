...
def pytest_addoption(parser):
    ...
    group.addoption(
        "--max-worker-restart",
        "--max-subordinate-restart",
        action="store",
        default=None,
        dest="maxworkerrestart",
        help="maximum number of workers that can be restarted "
        "when crashed (set to zero to disable this feature)\n"
        "'--max-subordinate-restart' option is deprecated and will be removed in "
        "a future release",
    )
    group.addoption(
        "--dist",
...
def worker_id(request):
    """Return the id of the current worker ('gw0', 'gw1', etc) or 'main'
    if running on the main node.
    """
