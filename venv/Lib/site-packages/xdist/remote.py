...
class WorkerInteractor(object):
    ...
    def pytest_collectreport(self, report):
        # send only reports that have not passed to main as optimization (#330)
...
def serialize_warning_message(warning_message):
    if isinstance(warning_message.message, Warning):
        ...
        # if not, we will just use the exception message on the main node
        try:
...
if __name__ == "__channelexec__":
    ...
    config.subordinateinput = config.workerinput
    config.subordinateoutput = config.workeroutput
