...
class MainQA(BaseCase):

    def setUp(self):
        self.check_count = 0
        self.auto_close_results_page = False
        super(MainQA, self).setUp(mainqa_mode=True)
    ...
    def verify(self, *args):
        warn_msg = "\nWARNING: MainQA skips manual checks in headless mode!"
        self.check_count += 1
        if self.headless:
        ...
        if use_jqc:
            ...
            if self.browser == 'ie':
                ...
                self.execute_script('''if(confirm("%s"))
                    {window.main_qa_result="Success!"}
                    else{window.main_qa_result="Failure!"}''' % question)
                time.sleep(0.05)
                self.__wait_for_special_alert_absent()
                text = self.execute_script('return window.main_qa_result')
            else:
                try:
                    self.execute_script(
                        '''if(confirm("%s"))
